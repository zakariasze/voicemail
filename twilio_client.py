...
def send_sms(to_number: str, body: str) -> str:
    """Send one SMS via Twilio. Returns the Twilio ``MessageSid``.

    Uses ``TWILIO_FROM_NUMBER`` as the sender — the number must be
    SMS-capable (A2P 10DLC registered for US long-codes).
    """
    if not to_number:
        raise ValueError("to_number is required")
    if not body:
