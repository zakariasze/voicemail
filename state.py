...
def mark_sms_sent(call_sid: str) -> None:
    """Stamp ``sms_sent_at`` so the follow-up SMS isn't sent twice."""
    if not call_sid:
        raise ValueError("call_sid is required")
    now = _now()
    with _connect() as conn:
        conn.execute(
            "UPDATE calls SET sms_sent_at=?, updated_at=? "
            "WHERE call_sid=?",
            (now, now, call_sid),
        )
...
def mark_forwarded(
    ...
    *,
    ...
    """Record the outcome of the call-forwarding dial attempt.

    ``forwarded_to`` is the E.164 number that answered, or ``None`` if
    nobody picked up. ``forward_status`` is the raw ``DialCallStatus``
    from Twilio (``completed``, ``no-answer``, ``busy``, ``failed``).
    """
    if not call_sid:
        raise ValueError("call_sid is required")
    now = _now()
    with _connect() as conn:
        conn.execute(
            "UPDATE calls SET forwarded_to=?, forward_status=?, updated_at=? "
            "WHERE call_sid=?",
            (forwarded_to, forward_status, now, call_sid),
        )
